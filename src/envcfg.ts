...
function log(msg: string, level: "Verbose" | "Log" | "Error" | "Warning" | "Success" = "Verbose") {
    if (level == "Verbose") console.log(chalk.cyan(`[ ${new Date().toISOString()}]`), chalk.gray("[ENVINIT]"), chalk.gray(msg));
    if (level == "Log") console.log(chalk.cyan(`[ ${new Date().toISOString()}]`), chalk.gray("[ENVINIT]"), chalk.white(msg));
    if (level == "Error") console.log(chalk.cyan(`[ ${new Date().toISOString()}]`), chalk.gray("[ENVINIT]"), chalk.red(msg));
    if (level == "Warning") console.log(chalk.cyan(`[ ${new Date().toISOString()}]`), chalk.gray("[ENVINIT]"), chalk.yellow(msg));
    if (level == "Success") console.log(chalk.cyan(`[ ${new Date().toISOString()}]`), chalk.gray("[ENVINIT]"), chalk.green(msg));
}

function initResilience() {
    log(`Setting up uncaughtException handler...`, "Log");
    process.on("uncaughtException", (err) => {
        log(`Uncaught exception: ${util.inspect(err, { depth: null})}`, "Error");
    });
    log(`uncaughtException handler registered!`, "Success");
    
    log(`Setting up unhandledRejection handler...`, "Log");
    process.on("unhandledRejection", (err) => {
        log(`Unhandled rejection: ${util.inspect(err, { depth: null})}`, "Error");
    });
    log(`unhandledRejection handler registered!`, "Success");
}
...
function initEnv(): Promise<boolean> {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            log(`Environment configuration success!`, "Success");
            resolve(true);
        } catch (error) {
            log(`Environment configuration encountered a problem!`, "Warning");
            reject(error);
        } finally {
            log(`Setting up resilience...`, "Verbose");
            initResilience();
            log(`Resilience parameters configured!`, "Success");
            log(`Environment configuration completed.`, "Log");
        }
    });
}
